...
const Settings = ({ navigation }:any) => {  
	...
	const submitPassword = async () => {
		if (loading) return;
		if (status.currentPassword.length < 6 || status.currentPassword.length > 32) {
			showToast("Invalid password length, expected 6 ~ 32 characters", "error");
			refPassword1?.current?.focus();
			return;
		}
		if (status.newPassword1.length < 6 || status.newPassword1.length > 32) {
			showToast("Invalid password length, expected 6 ~ 32 characters", "error");
			updateStatus({errmsg : '', success : ''})
			refPassword2?.current?.focus();
			return;
		}	
		if (status.newPassword2.length < 6 || status.newPassword2.length > 32) {
			showToast("Invalid password length, expected 6 ~ 32 characters", "error");
			...
			<View >
				<View style = {styles.backpanel}> 
					<TouchableOpacity onPress={goSupport}  style={styles.settingrow}> 
						<View style={{flex:2}}> 
							<Icons.support width={w(6)} height={w(6)} color={Colors.Primary} />
						</View>
						<View style={{flex:7}}> 
					...
					<TouchableOpacity onPress={goPincode} style={styles.settingrow}> 
						<View style={{flex:2}}> 
							<Icons.resetPincode width={w(6)} height={w(6)} color={Colors.Primary}/>
						</View>
						<View style={{flex:7}}> 
					...
					<TouchableOpacity onPress={goPassword} style={styles.settingrow}> 
						<View style={{flex:2}}> 
							<Icons.resetPassword width={w(6)} height={w(6)} color={Colors.Primary}/>
						</View>
						<View style={{flex:7}}> 
					...
					<TouchableOpacity onPress={onLogout} style={styles.settingrow}> 
						<View style={{flex:2}}> 
							<Icons.logout width={w(6)} height={w(6)} color={Colors.Primary}/>
						</View>
						<View style={{flex:7}}> 
